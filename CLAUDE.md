...
## 4. Chart of Accounts — THREE unreconciled versions exist

`[unclear — confirm]` **Do not treat any one version below as authoritative without asking the client.** See `docs/planning/kayani-erp-full-export.md` section 4.1 for the full line-by-line diff between the first two; this repo's schema is a third.

1. **A memory-held earlier version** (planning-account chat memory, source/date unknown) — has "Softwares/Computer & Hardware," "Income Tax Withheld"/"GST Withheld" under *Current Assets*.
2. **The 2026-09-07 client spreadsheet version** (`docs/planning/kayani-erp-full-export.md` section 2, sourced from `chart of accounts.xlsx`) — has "ERP, Computer & Hardware" instead, moves the Withheld/Payable accounts to *Income/(Loss)* marked status-of-party-wise, and adds "JS Kiyan Traders" (bank account), LPP provision accounts (in two different sections), "Income Tax Payable"/"GST Payable" under *Equity & Reserves* (a **confirmed** duplicate-looking pair alongside the Income/(Loss)-section Withheld/Payable accounts — verified present in the real file, not a transcription error; still flag to client, don't assume it's intentional), and "Travelling Expense Account." **Directly verified against the actual spreadsheet cell-by-cell 2026-09-08 — exact match, strongest provenance of the three versions since it's a real client file, not a relayed summary.** Recommend treating this as the working default once the client confirms.
3. **This repo's built/seeded version** (`src/db/schema/accounts.ts`, seeded via `src/db/seed.ts`) — a *shorter* version matching neither of the above exactly: no "Security Deposits" wording difference, no "JS Kiyan Traders," no LPP provision accounts, no "Travelling Expense," no party-wise/LIFO annotations on individual line items (LIFO is captured only as a general note, not per-account).

**What's actually built and running locally right now** (version 3): flat structure, single flat `chart_of_accounts` table, entity differentiation **only** at the bank-account level (5 bank rows tagged with `legal_entity_id`, every other row `null`) — this part of the design is confirmed correct regardless of which COA content-version wins, and is validated against a real local Postgres (see section 8). What needs client confirmation is the *line-item content*, not this structural approach.
...
### 5.5 Party Form (Form "D") — customers & vendors, not built yet
...
### 5.10 Sales document chain — not built yet
...
- Chart of accounts: three non-matching content-versions (section 4).
